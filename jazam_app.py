...
def valid_moves(G):
    ...
    for d,tgt in [("cw",t_cw),("ccw",t_ccw)]:
        ...
        toll_on_this=(d=="cw" and G["toll_cw"]) or (d=="ccw" and G["toll_ccw"])
...
def check_end(G, mode):
    ...
    for p in range(2):
        ...
        if pc["black"]<=0 and pc["white"]<=0 and pc["blue"]<=0:
            ...
            add_log(G,who,"sin piezas · gana quien tiene más puntos" if ES else "out of beads · most points wins",0); return
    # Also end if no moves available
...
with tab_game:
    ...
    if G["over"]:
        ...
        reason_txt=""
        if reason=="nopcs": reason_txt=f" · {'piezas agotadas' if ES else 'beads exhausted'}"
