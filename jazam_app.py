...
def do_play(G, piece_type, direction, mode):
    ...
    if piece_type == "blue":
        ...
        G["lv"] = new_lv
        # Both pointers forced to aligned space — player MUST place there first
        G["ptr_cw"] = aligned
        G["ptr_ccw"] = aligned
        G["forced_space"] = aligned   # flag: first move in new level must be here
        G["started"] = True
        G["to_place"] = 1
        G["placed_this_turn"] = 0
        # same player repeats — no cp switch
    ...
    # Bono arquitecto + avance de nivel al completar
