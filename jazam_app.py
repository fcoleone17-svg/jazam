...
def init_game():
    return {
        ...
        # marks[li][si] = None | "blocking" | "normal"
        # Set when a white is played — marks the next 2 spaces in that direction
        "marks": [[None]*n for n in LEVELS],
        "lv": 0,
        "ptr_cw": 1,
        "ptr_ccw": LEVELS[0]-1,
        "started": False,
        "forced_space": None,  # when set, player must place here first
        "to_place": 1,
        "placed_this_turn": 0,
        "last_color": None,
...
def get_mark(G, lv, si):
    return G["marks"][lv][si]
...
def valid_moves(G):
    ...
    # Forced space after blue jump — must place black or white here
    if G.get("forced_space") is not None:
        moves = []
    ...
    for d, tgt in [("cw", t_cw), ("ccw", t_ccw)]:
        ...
        mark = get_mark(G, lv, tgt)
        is_blocking = (mark == "blocking")
        cel = is_celestial(lv, tgt) and not is_blocking
        if pc["black"] > 0: moves.append(("black", d))
        if pc["white"] > 0 and not is_blocking: moves.append(("white", d))
...
def do_play(G, piece_type, direction, mode):
    ...
    if piece_type == "blue":
        ...
        G["scores"][cp] += pts
        G["board"][lv][tgt] = {"p": cp, "t": "blue", "neutralized": False}
        G["marks"][lv][tgt] = None
        if direction == "cw": G["ptr_cw"] = (tgt+1)%n
        else: G["ptr_ccw"] = (tgt-1)%n
        add_log(G, who, f"azul {dn} Nv{lv+1}·esp{tgt+1} → sube Nv{lv+2}", pts)
        new_lv = lv+1
        if new_lv > 5: check_end(G, mode); return
        new_n = LEVELS[new_lv]
        aligned = aligned_si(lv, tgt, new_lv)
        G["lv"] = new_lv
        G["marks"][new_lv] = [None]*new_n  # clear any stale marks in new level
        G["ptr_cw"] = aligned
        G["ptr_ccw"] = aligned
    ...
    if direction == "start":
        ...
        target_si = G["forced_space"]
        G["forced_space"] = None  # clear after use
    elif direction == "cw":
        target_si = find_free_cw(G)
    else:
        target_si = find_free_ccw(G)
    if target_si is None: return

    mark = get_mark(G, lv, target_si)
    neutralized = (mark == "blocking")
    effective_type = "black" if neutralized else piece_type

    pc[piece_type] -= 1
    G["board"][lv][target_si] = {"p": cp, "t": piece_type, "neutralized": neutralized}
    G["marks"][lv][target_si] = None  # clear mark once filled
    G["last_color"] = effective_type
    G["placed_this_turn"] += 1

    if direction == "start":
        G["started"] = True
        G["ptr_cw"] = 1; G["ptr_ccw"] = (n-1)%n
    elif direction == "forced":
        # After forced placement, both pointers expand from that space
    ...
    add_log(G, who, f"{tn} {dn} Nv{lv+1}·esp{target_si+1}{note}", 0)

    # WHITE: marca los 2 espacios siguientes en esa dirección
    if effective_type == "white" and direction in ("cw","ccw"):
        s1 = (target_si+1)%n if direction=="cw" else (target_si-1)%n
        s2 = (s1+1)%n        if direction=="cw" else (s1-1)%n
        if G["board"][lv][s1] is None: G["marks"][lv][s1] = "blocking"
        if G["board"][lv][s2] is None: G["marks"][lv][s2] = "normal"

    # Bono arquitecto — solo niveles 1, 2 y 3 (índices 0, 1, 2)
    if lv <= 2 and all(x is not None for x in G["board"][lv]):
        G["scores"][cp] += 4
        add_log(G, who, f"Nv{lv+1} completo · +4" if ES else f"Lv{lv+1} complete · +4", 4)

    # Avance de nivel al completar (todos los niveles)
    if all(x is not None for x in G["board"][lv]):
        if lv < 5:
            G["lv"] = lv + 1
            new_n = LEVELS[lv + 1]
            G["marks"][lv+1] = [None]*new_n
            G["ptr_cw"] = 0
            G["ptr_ccw"] = new_n - 1

    # Turno: siempre pasa al otro jugador (1 pieza por turno)
    G["cp"] = 1 - cp
    G["placed_this_turn"] = 0
    G["to_place"] = 1
...
def check_end(G, mode):
    # Solo termina si alguien colocó una pieza en el centro (nivel 6, índice 5)
    if G["lv"] == 5:
        center_pieces = [x for x in G["board"][5] if x is not None]
        if center_pieces:
            who_idx = center_pieces[0]["p"]
            G["scores"][who_idx] += 16
            ES = G.get("_es", True)
...
def ai_move(G, mode):
    ...
    # Forced space after blue — just place black
    if G.get("forced_space") is not None:
        t = "black" if pc["black"]>0 else "white"
        do_play(G, t, "forced", mode); return

    # If next space is blocking, prefer black
    t_cw=find_free_cw(G); t_ccw=find_free_ccw(G)
    next_is_blocking = any(
        G["marks"][lv][tgt] == "blocking"
        for tgt in [t_cw, t_ccw] if tgt is not None
    )
    if next_is_blocking:
...
def render_board_svg(G):
    ...
    nlv=G["lv"]
    forced=G.get("forced_space")
    t_cw=find_free_cw(G) if G["started"] and forced is None else None
    t_ccw=find_free_ccw(G) if G["started"] and forced is None else None
    first_space=(not G["started"])
    ...
    for li in range(6):
        ...
        for si in range(n):
            ...
            is_cw_target=(li==nlv and si==t_cw and not G["over"] and cell is None and G["started"] and t_cw is not None and t_cw!=t_ccw)
            is_ccw_target=(li==nlv and si==t_ccw and not G["over"] and cell is None and G["started"] and t_ccw is not None and t_cw!=t_ccw)
            is_first=(li==nlv and si==0 and first_space and not G["over"])
            is_blocking_mark=(mark=="blocking" and cell is None)
            is_normal_mark=(mark=="normal" and cell is None)
            ...
            elif is_blocking_mark: fill,stroke,sw="rgba(230,100,30,0.18)","#D85A30",2
            elif is_normal_mark: fill,stroke,sw="rgba(186,117,23,0.12)","#BA7517",1.5
...
with tab_rules:
    ...
    if ES:
        ...
        c1,c2=st.columns(2)
        with c1: st.markdown('<div class="rule-box">🏛️ <b>Bono Arquitecto (+4)</b><br>Solo en los primeros 3 niveles · al completarlos da +4 pts.</div>',unsafe_allow_html=True)
...
with tab_game:
    ...
    if not G["over"]:
        ...
        cw_blocking=(G["marks"][lv][t_cw]=="blocking") if t_cw is not None else False
        ccw_blocking=(G["marks"][lv][t_ccw]=="blocking") if t_ccw is not None else False
        any_blocking=cw_blocking or ccw_blocking
        ...
        if not G["started"]:
            ...
            bar_cls="status-bar first"
            txt=f"🟢 <b>{name}</b> — {'subió de nivel · debe colocar en esp' if ES else 'went up · must place at sp'}{forced+1} · {'Nv' if ES else 'Lv'}{lv+1}"
        elif any_blocking:
            bar_cls="status-bar blocking"
            dirs=("↻ " if cw_blocking else "")+("↺" if ccw_blocking else "")
            txt=f"⚠️ <b>{name}</b> — {'zona de bloqueo' if ES else 'blocking zone'} {dirs} · {'pieza pierde propiedades' if ES else 'bead loses properties'}"
        elif not moves:
    ...
    if not G["over"]:
        ...
        if is_human and moves:
            ...
            elif G.get("forced_space") is not None:
                fsp=G["forced_space"]
                st.markdown(f"**{'Coloca en esp' if ES else 'Place at sp'}{fsp+1} · {'Nv' if ES else 'Lv'}{lv+1}**")
                c1,c2=st.columns(2)
                with c1:
                    if st.button(f"⚫ {'Negra' if ES else 'Black'} esp{fsp+1} ({pc['black']})",disabled=not can("black","forced"),use_container_width=True,key="bk_f"):
                        do_play(G,"black","forced",mode); st.rerun()
                with c2:
                    if st.button(f"🟡 {'Blanca' if ES else 'White'} esp{fsp+1} ({pc['white']})",disabled=not can("white","forced"),use_container_width=True,key="bw_f"):
