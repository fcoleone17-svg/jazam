        ...
        if is_human and moves:
            ...
            # ── Turno normal: flujo 2 pasos ──
                ...
                # Paso 2: elegir dirección
                    ...
                    st.markdown(f"**{lbl_t} — {'¿↻ o ↺?' if ES else '↻ or ↺?'}**")
                    c1,c2,c3=st.columns([2,1,2])
                    with c1:
                        lbl=f"↻ esp{t_cw2+1}" if t_cw2 is not None else "↻"
                        if st.button(lbl,disabled=not can(t,"cw"),use_container_width=True,key="dir_cw"):
                    ...
                    with c3:
                        ...
                        if st.button(lbl,disabled=not can(t,"ccw"),use_container_width=True,key="dir_ccw"):
