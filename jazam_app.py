...
def render_board_svg(G):
    ...
    for li in range(6):
        n=LEVELS[li]
        # Tamaño de dot según nivel — más grande para mejor visibilidad móvil
        if li==5: x,y=cx,cy; r_dot=24
        else:
            r_dot=[13,12,11,11,11,0][li]
        ...
        for si in range(n):
            ...
            if li==5: fill,stroke,sw="#DDF0CC","#3B6D11",2.5
            elif is_forced: fill,stroke,sw="rgba(59,109,17,0.35)","#3B6D11",3
            elif is_first: fill,stroke,sw="rgba(59,109,17,0.25)","#3B6D11",2.5
            elif is_cw or is_ccw: fill,stroke,sw="rgba(255,107,0,0.25)","#FF6B00",2.5
            elif cel: fill,stroke,sw="#C5E8FF","#185FA5",2
            else: fill,stroke,sw="rgba(80,60,20,0.05)","rgba(80,60,20,0.18)",0.8
            ...
            if cel and not cell and not is_cw and not is_ccw and not is_forced and not is_first:
                pts=space_pts(li,si)
                lines.append(f'<text x="{x:.1f}" y="{y+3:.1f}" text-anchor="middle" font-size="{8 if li==0 else 9}" font-weight="600" fill="#0C447C" font-family="DM Sans,sans-serif">{pts}</text>')
            if li==5 and not any(x2 is not None for x2 in G["board"][5]):
                lines.append(f'<text x="{cx}" y="{cy+5}" text-anchor="middle" font-size="13" font-weight="600" fill="#27500A" font-family="DM Sans,sans-serif">12</text>')
            ...
            if cell:
                ...
                if neu or t2=="black": pf,ps="#E8E4D9","#999990"
                ...
                # Punto de color del jugador — más grande y visible
                pd=P_COLOR[cell["p"]]
                lines.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="3.5" fill="{pd}"/>')
