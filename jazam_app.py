...
def aligned_entry(board, lv_old, last_si, lv_new):
    """Espacio de entrada al nuevo nivel: alineado al último espacio jugado,
    si está ocupado busca el más cercano en sentido horario."""
    new_n = LEVELS[lv_new]
    aligned = aligned_si(lv_old, last_si, lv_new)
    for offset in range(new_n):
        candidate = (aligned + offset) % new_n
        if board[lv_new][candidate] is None:
            return candidate
    return 0  # fallback (nivel lleno, no debería pasar)
    ...
    if paying_toll_first:
        ...
        if all(x is not None for x in G["board"][lv]):
            if lv<=2:
                ...
                add_log(G,who,f"Nv{lv+1} completo · +4" if ES else f"Lv{lv+1} complete · +4",4)
            if lv<5:
                entry=aligned_entry(G["board"],lv,target,lv+1)
                G["lv"]=lv+1; new_n=LEVELS[lv+1]
                G["ptr_cw"]=(entry+1)%new_n; G["ptr_ccw"]=(entry-1)%new_n
                clear_tolls(G)
                # Obs2: conservar obligación — jugador coloca 2ª pieza en nuevo nivel
                # toll_placing sigue activo, forced_space marca dónde empieza
                G["forced_space"]=entry
                # No cancelar toll_placing — el jugador aún debe colocar la 2ª pieza
                check_end(G,mode); return
        check_end(G,mode); return
