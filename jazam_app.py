...
CELESTIALS = {
    0: {0:6, 8:6, 16:6, 24:6},   # Nv1: 12:00, 3:00, 6:00, 9:00
    1: {0:4, 4:4, 8:4, 12:4},    # Nv2: 12:00, 3:00, 6:00, 9:00
    2: {0:2, 2:2, 4:2, 6:2},     # Nv3: 12:00, 3:00, 6:00, 9:00
}
...
def init_game():
    ...
    return {
        ...
        "started": False,
        "forced_space": None,
        "last_was_blue": False,  # True si la última jugada fue azul (no se puede repetir)
...
def valid_moves(G):
    ...
    for d,tgt in [("cw",t_cw),("ccw",t_ccw)]:
        ...
        if pc["black"]>0: moves.append(("black",d))
        if pc["white"]>0: moves.append(("white",d))
        if pc["blue"]>0 and cel and lv<=2 and not G.get("last_was_blue",False): moves.append(("blue",d))
    seen=set(); unique=[]
    for m in moves:
...
def do_play(G, piece_type, direction, mode):
    ...
    if piece_type=="blue" and G["toll_placing"] is None and G["forced_space"] is None and not is_toll_first:
        ...
        G["started"]=True
        G["last_was_blue"]=True  # bloquea otra azul en el turno repetido
