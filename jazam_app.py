...
def do_play(G, piece_type, direction, mode):
    ...
    if piece_type == "blue":
        ...
        new_lv = lv+1; new_n = LEVELS[new_lv]
        if new_lv > 5: return  # safety check
        aligned = aligned_si(lv, tgt, new_lv)
        G["lv"] = new_lv
        G["ptr_cw"] = (aligned+1)%new_n
        G["ptr_ccw"] = (aligned-1)%new_n
        G["board"][new_lv][aligned] = {"p": cp, "t": "blue", "neutralized": False}
        G["started"] = True
        G["to_place"] = 1; G["placed_this_turn"] = 0
