...
def init_game():
    ...
    return {
        ...
        "lv": 0,
        "ptr_cw": 1,         # next free space going ↻
        "ptr_ccw": n0 - 1,   # next free space going ↺
        "toll_cw": False,    # True = next ↻ move is toll (no properties)
        "toll_ccw": False,   # True = next ↺ move is toll (no properties)
        "toll_placing": None,# None | "cw" | "ccw" — currently paying toll, which side
        "started": False,
        "forced_space": None,
...
def valid_moves(G):
    ...
    if G["toll_placing"] is not None:
        ...
        if pc["blue"]>0 and cel and lv<=2: moves.append(("blue",d))
        return moves
    ...
    for d,tgt in [("cw",t_cw),("ccw",t_ccw)]:
        ...
        # In toll mode for this direction: can only play black/white/blue (but first piece has no props)
        # — valid_moves still shows all options, neutralization happens in do_play
        cel=is_celestial(lv,tgt)
        if pc["black"]>0: moves.append(("black",d))
        if pc["white"]>0: moves.append(("white",d))
        if pc["blue"]>0 and cel and lv<=2: moves.append(("blue",d))
...
def do_play(G, piece_type, direction, mode):
    ...
    # ── BLUE (only valid when not in toll first-piece) ──
    if piece_type=="blue" and G["toll_placing"] is None and G["forced_space"] is None:
        tgt=find_free_cw(G) if direction=="cw" else find_free_ccw(G)
        if tgt is None: return
        pc["blue"]-=1
        pts=space_pts(lv,tgt)
        G["scores"][cp]+=pts
        ...
        G["toll_cw"]=False; G["toll_ccw"]=False
    ...
    paying_toll_first = (G["toll_placing"] is None and direction in ("cw","ccw") and
                         ((direction=="cw" and G["toll_cw"]) or
                          (direction=="ccw" and G["toll_ccw"])))
    ...
    if paying_toll_first:
        # First toll piece placed — now same player places second (with properties)
        if direction=="cw": G["toll_cw"]=False
        else: G["toll_ccw"]=False
        G["toll_placing"]=direction  # flag: next piece has properties
        check_end(G,mode); return  # same player continues

    if G["toll_placing"] is not None:
        # Just placed the 2nd toll piece (with properties) — now apply white effect if needed
        G["toll_placing"]=None
        if effective=="white":
            # White activates toll on this same direction for next player
            d=direction
            if d=="cw": G["toll_cw"]=True
            else: G["toll_ccw"]=True
        elif effective=="blue":
            # Blue in 2nd toll position — goes up normally
            ...
            check_end(G,mode); return  # same player repeats for blue
        # Pass turn
        G["cp"]=1-cp
        check_end(G,mode); return

    # ── Normal piece effects ──
    if effective=="white" and direction in ("cw","ccw"):
...
def check_end(G, mode):
    if G["lv"]==5 and any(x is not None for x in G["board"][5]):
        ...
        add_log(G,who,"¡CENTRO! +10" if ES else "CENTER! +10",10)
        G["over"]=True
...
def ai_move(G, mode):
    ...
    if G["toll_placing"] is not None:
        d=G["toll_placing"]
        t_cw2=find_free_cw(G) if d=="cw" else None
        t_ccw2=find_free_ccw(G) if d=="ccw" else None
        tgt=t_cw2 or t_ccw2
        cel=is_celestial(lv,tgt) if tgt else False
        if pc["blue"]>0 and cel and lv<=2: do_play(G,"blue",d,mode); return
        t="black" if pc["black"]>0 else "white"
    ...
    if black:
        ...
        for t,d in black:
            tgt=find_free_cw(G) if d=="cw" else find_free_ccw(G)
            if tgt is None: continue
            for dist in range(1,LEVELS[lv]):
                cand=(tgt+dist)%LEVELS[lv] if d=="cw" else (tgt-dist)%LEVELS[lv]
...
def render_board_svg(G):
    ...
    for li in range(6):
        ...
        for si in range(n):
            ...
            elif is_cw or is_ccw: fill,stroke,sw="rgba(186,117,23,0.2)","#BA7517",2.5
            ...
            if li==5 and not any(x2 is not None for x2 in G["board"][5]):
                lines.append(f'<text x="{cx}" y="{cy+5}" text-anchor="middle" font-size="12" font-weight="600" fill="#27500A" font-family="DM Sans,sans-serif">10</text>')

            if cell:
                t2=cell["t"]; neu=cell.get("neu",False); pr=r_dot-2.5
...
with tab_rules:
    ...
    if ES:
        ...
        st.markdown('<div class="rule-box">🟡 Cuando se juega una <b>blanca</b>, el extremo donde se jugó queda con <b>peaje activo</b>. El próximo jugador que quiera avanzar por ese extremo debe colocar <b>2 piezas</b>:<br>• <b>1ª pieza</b>: rellena el espacio pero <b>pierde sus propiedades</b> (una azul no sube de nivel, una blanca no activa peaje).<br>• <b>2ª pieza</b>: funciona con plenas propiedades y determina la secuencia siguiente.<br>El peaje lo paga <b>cualquier jugador</b> que avance por ese extremo, no necesariamente el rival.</div>',unsafe_allow_html=True)
        st.divider()
        ...
        with c2: st.markdown('<div class="rule-box">🎯 <b>Centro (+10)</b><br>Llegar al centro termina el juego. Gana quien tenga más puntos.</div>',unsafe_allow_html=True)
        ...
        with c2: st.markdown('<div class="rule-box" style="text-align:center;"><b>🟡 8 White</b><br><small>Fills 1 space · activates toll on that end: next player advancing there places 2 (first has no properties)</small></div>',unsafe_allow_html=True)
        with c3: st.markdown('<div class="rule-box" style="text-align:center;"><b>🔵 2 Blue</b><br><small>Celestial only · go up a level · repeat turn</small></div>',unsafe_allow_html=True)
        st.divider()
        st.markdown("### The White Toll")
        st.markdown('<div class="rule-box">🟡 When a <b>white</b> is played, that end gets a <b>toll</b>. The next player advancing there must place <b>2 beads</b>:<br>• <b>1st bead</b>: fills the space but <b>loses its properties</b> (blue won\'t go up, white won\'t activate toll).<br>• <b>2nd bead</b>: works normally and determines the next sequence.<br>The toll applies to <b>any player</b> advancing that end, not just the opponent.</div>',unsafe_allow_html=True)
        st.divider()
        st.markdown("### Scoring")
        st.markdown("| Level | Celestials | Points |\n|-------|------------|--------|\n| 1 | 3:00, 6:00, 9:00 | 6 pts |\n| 2 | sp 3, 7, 11, 15 | 4 pts |\n| 3 | 12:00, 3:00, 6:00, 9:00 | 2 pts |")
        c1,c2=st.columns(2)
        ...
        with c2: st.markdown('<div class="rule-box">🎯 <b>Center (+10)</b><br>Reaching center ends the game. Most points wins.</div>',unsafe_allow_html=True)
    ...
    if not G["over"]:
        ...
        if is_human and moves:
            ...
            if not G["started"]:
                ...
                c1,c2=st.columns(2)
                with c1:
                    if st.button(f"⚫ 12:00 ({pc['black']})",disabled=not can("black","start"),use_container_width=True,key="bk_st"):
                        do_play(G,"black","start",mode); st.rerun()
                with c2:
                    if st.button(f"🟡 12:00 ({pc['white']})",disabled=not can("white","start"),use_container_width=True,key="bw_st"):
                        do_play(G,"white","start",mode); st.rerun()

            elif forced is not None:
                fsp=forced; lv2=G["lv"]
                st.markdown(f"**{'Coloca en esp' if ES else 'Place at sp'}{fsp+1} · {'Nv' if ES else 'Lv'}{lv2+1}:**")
                c1,c2=st.columns(2)
                with c1:
                    if st.button(f"⚫ esp{fsp+1} ({pc['black']})",disabled=not can("black","forced"),use_container_width=True,key="bk_f"):
                        do_play(G,"black","forced",mode); st.rerun()
                with c2:
                    if st.button(f"🟡 esp{fsp+1} ({pc['white']})",disabled=not can("white","forced"),use_container_width=True,key="bw_f"):
                        do_play(G,"white","forced",mode); st.rerun()
                ...
                c1,c2,c3=st.columns(3)
                with c1:
                    if st.button(f"⚫ {d_sym} ({pc['black']})",disabled=not can("black",toll_placing),use_container_width=True,key="bk_t"):
                        do_play(G,"black",toll_placing,mode); st.rerun()
                with c2:
                    if st.button(f"🟡 {d_sym} ({pc['white']})",disabled=not can("white",toll_placing),use_container_width=True,key="bw_t"):
                        do_play(G,"white",toll_placing,mode); st.rerun()
                with c3:
                    if st.button(f"🔵 {d_sym} ({pc['blue']})",disabled=not can("blue",toll_placing),use_container_width=True,key="bb_t"):
                        do_play(G,"blue",toll_placing,mode); st.rerun()

            else:
                st.markdown(f"**{'Elige tu jugada:' if ES else 'Choose your move:'}**")
                c1,c2=st.columns(2)
                with c1:
                    if st.button(f"⚫ ↻ ({pc['black']})",disabled=not can("black","cw"),use_container_width=True,key="bk_cw"):
                        do_play(G,"black","cw",mode); st.rerun()
                with c2:
                    if st.button(f"⚫ ↺ ({pc['black']})",disabled=not can("black","ccw"),use_container_width=True,key="bk_ccw"):
                        do_play(G,"black","ccw",mode); st.rerun()
                c1,c2=st.columns(2)
                with c1:
                    if st.button(f"🟡 ↻ ({pc['white']})",disabled=not can("white","cw"),use_container_width=True,key="bw_cw"):
                        do_play(G,"white","cw",mode); st.rerun()
                with c2:
                    if st.button(f"🟡 ↺ ({pc['white']})",disabled=not can("white","ccw"),use_container_width=True,key="bw_ccw"):
                        do_play(G,"white","ccw",mode); st.rerun()
                if can("blue","cw") or can("blue","ccw"):
                    c1,c2=st.columns(2)
                    with c1:
                        if st.button(f"🔵 ↻ ({pc['blue']})",disabled=not can("blue","cw"),use_container_width=True,key="bb_cw"):
                            do_play(G,"blue","cw",mode); st.rerun()
                    with c2:
                        if st.button(f"🔵 ↺ ({pc['blue']})",disabled=not can("blue","ccw"),use_container_width=True,key="bb_ccw"):
