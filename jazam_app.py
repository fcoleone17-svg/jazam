...
# Puntos por celeste según posición en cada nivel
CEL_PTS = {
    0: {4:3, 12:6, 20:9, 28:12},   # Nivel 1: esp5=3, esp13=6, esp21=9, esp29=12
    1: {0:8, 4:2, 8:4, 12:6},      # Nivel 2: esp1=8, esp5=2, esp9=4, esp13=6
    2: {1:4, 3:1, 5:2, 7:3},       # Nivel 3: esp2=4, esp4=1, esp6=2, esp8=3
}
...
def space_pts(li, si):
    if li == 5: return 16
    return CEL_PTS.get(li, {}).get(si, 0)
...
def init_game():
    ...
    for li, n in enumerate(LEVELS):
        levels_data.append({
            "front_cw": 0,        # next space to fill going clockwise (index advances)
            "front_ccw": n - 1,   # next space to fill going counter-clockwise (index decreases)
            "closed_cw": False,   # clockwise front permanently blocked by white
            "closed_ccw": False,  # ccw front permanently blocked by white
            "filled": set(),      # set of filled space indices
            "blocked": set(),     # set of blocked space indices (white beads)
        })
    return {
        "cp": 0, "scores": [0, 0],
...
def fronts_meet(lv_data, li):
    """Check if the two fronts have met — no empty space between them."""
    n = LEVELS[li]
    fcw = lv_data["front_cw"]
    fccw = lv_data["front_ccw"]
    filled = lv_data["filled"]
    blocked = lv_data["blocked"]
    occupied = filled | blocked
    # Fronts meet if there's no unoccupied space between fcw and fccw going clockwise
    # (i.e., every space from 0 to n-1 is occupied or blocked, or the fronts have crossed)
    if lv_data["closed_cw"] and lv_data["closed_ccw"]:
        return True
    # Check all spaces between the two fronts
    if fcw > fccw:
        return True  # fronts have crossed
    for i in range(fcw, fccw + 1):
...
def get_moves(G):
    """
    Returns list of (piece_type, front) where front is 'cw' or 'ccw'.
    Also includes blue moves as (piece_type, direction) where direction is 'up' or 'down'.
    """
    ...
    # Black moves: advance a front
    if pc["black"] > 0:
        if not lv["closed_cw"]:
            fcw = lv["front_cw"]
            # next space to fill clockwise
            next_cw = fcw
            while next_cw < n and next_cw in (lv["filled"] | lv["blocked"]):
                next_cw += 1
            fccw = lv["front_ccw"]
            while fccw >= 0 and fccw in (lv["filled"] | lv["blocked"]):
                fccw -= 1
            if next_cw <= fccw:  # there's still space
                moves.append(("black", "cw"))
        if not lv["closed_ccw"]:
            fccw = lv["front_ccw"]
            next_ccw = fccw
            while next_ccw >= 0 and next_ccw in (lv["filled"] | lv["blocked"]):
                next_ccw -= 1
            fcw = lv["front_cw"]
            while fcw < n and fcw in (lv["filled"] | lv["blocked"]):
                fcw += 1
            if next_ccw >= fcw:
                moves.append(("black", "ccw"))

    # White moves: close a front permanently
    if pc["white"] > 0:
        if not lv["closed_cw"]:
            moves.append(("white", "cw"))
        if not lv["closed_ccw"]:
            moves.append(("white", "ccw"))
...
def get_next_front_space(lv_data, li, front):
    """Get the next fillable space for the given front."""
    n = LEVELS[li]
    occupied = lv_data["filled"] | lv_data["blocked"]
    if front == "cw":
        si = lv_data["front_cw"]
        while si < n and si in occupied:
            si += 1
        return si if si < n else None
    else:
        si = lv_data["front_ccw"]
        while si >= 0 and si in occupied:
            si -= 1
...
def do_play(G, t, d, mode):
    ...
    if t == "black":
        ...
        G["cur_lv"] = new_li
        # Set both fronts of the new level to start from the aligned space
        new_lv = G["levels"][new_li]
        new_lv["front_cw"] = new_si
        new_lv["front_ccw"] = new_si
...
with tab_rules:
    ...
    if ES:
        ...
        with c1: st.markdown('<div class="rule-box" style="text-align:center;"><b>⚫ 14 Negras</b><br><small>Avanza 1 espacio desde el frente elegido (→ o ←)</small></div>', unsafe_allow_html=True)
        with c2: st.markdown('<div class="rule-box" style="text-align:center;"><b>🟡 8 Blancas</b><br><small>Cierra permanentemente el frente elegido — nadie puede seguir por ahí</small></div>', unsafe_allow_html=True)
        ...
        c1,c2,c3 = st.columns(3)
        with c1: st.markdown('<div class="rule-box" style="text-align:center;"><b>⚫ 14 Black</b><br><small>Advance 1 space from chosen front (→ or ←)</small></div>', unsafe_allow_html=True)
        with c2: st.markdown('<div class="rule-box" style="text-align:center;"><b>🟡 8 White</b><br><small>Permanently close the chosen front — nobody can advance there</small></div>', unsafe_allow_html=True)
...
with tab_game:
    ...
    if not G["over"]:
        ...
        if not moves:
            ...
            pts=space_pts(li,fcw) if fcw_cel else space_pts(li,fccw)
            cl_idx=(fcw if fcw_cel else fccw)//(LEVELS[li]//4) if li<3 else 0
            cl=["12:00","3:00","6:00","9:00"][cl_idx]
            bar_cls="status-bar celestial"
            txt=f"★ <b>{name}</b> — {'Frente celeste' if ES else 'Celestial front'} {cl} · {pts}pts · {'Nv' if ES else 'Lv'}{li+1}"
        else:
            bar_cls="status-bar"
            cw_info=f"→ esp{fcw+1}" if not lv['closed_cw'] else ("→ ✕" if ES else "→ ✕")
            ccw_info=f"← esp{fccw+1}" if not lv['closed_ccw'] else ("← ✕" if ES else "← ✕")
