        ...
        if is_human and moves:
            ...
            # ── Turno normal: 2 pasos ──
                ...
                # Paso 2: elegir dirección
                    ...
                    # Calcular posición de reloj aproximada para cada espacio
                    def clock_pos(si, lv):
                        n=LEVELS[lv]; angle=(si/n)*360
                        hours=["12:00","1:30","3:00","4:30","6:00","7:30","9:00","10:30"]
                        idx=round(angle/45)%8
                        return hours[idx]

                    lbl_cw=f"↻ · {clock_pos(t_cw2,G['lv'])}" if t_cw2 is not None else "↻"
                    lbl_ccw=f"↺ · {clock_pos(t_ccw2,G['lv'])}" if t_ccw2 is not None else "↺"

                    # Animación pulsante en los espacios disponibles
                    st.markdown("""<style>
@keyframes pulse-ring {
  0%   { stroke-opacity: 0.9; r: 13px; }
  50%  { stroke-opacity: 0.3; r: 17px; }
  100% { stroke-opacity: 0.9; r: 13px; }
}
.pulse-target { animation: pulse-ring 1s ease-in-out infinite; }
</style>""", unsafe_allow_html=True)

                    st.markdown(f"**{lbl_t} — {'elige dirección:' if ES else 'choose direction:'}**")
                    c1,c2,c3=st.columns([5,1,5])
                    with c1:
                        if st.button(lbl_cw,disabled=not can(t,"cw"),use_container_width=True,key="dir_cw",type="primary"):
                    ...
                    with c3:
                        if st.button(lbl_ccw,disabled=not can(t,"ccw"),use_container_width=True,key="dir_ccw",type="primary"):
