    ...
    if not G["over"]:
        ...
        if is_human and moves:
            ...
            # ── Turno normal: 2 pasos ──
                ...
                # Paso 2: elegir dirección con botones claros
                else:
                    t=selected
                    lbl_t={"black":"⚫","white":"🟡","blue":"🔵"}[t]
                    st.markdown(f"**{lbl_t} — {'¿dónde colocas?' if ES else 'where?'}**")
                    c1,c2,c3=st.columns([5,1,5])
                    with c1:
                        lbl=f"↻  esp {t_cw2+1}" if t_cw2 is not None else "↻"
                        if st.button(lbl,disabled=not can(t,"cw"),use_container_width=True,key="dir_cw",type="primary"):
                            do_play(G,t,"cw",mode); st.session_state.pop("selected_color",None); st.rerun()
                    with c2:
                        if st.button("✕",use_container_width=True,key="cancel_sel"):
                            st.session_state.pop("selected_color",None); st.rerun()
                    with c3:
                        lbl=f"↺  esp {t_ccw2+1}" if t_ccw2 is not None else "↺"
                        if st.button(lbl,disabled=not can(t,"ccw"),use_container_width=True,key="dir_ccw",type="primary"):
