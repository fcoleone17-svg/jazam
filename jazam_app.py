...
def get_moves(G):
    ...
    def space_between(a, b):
        """True if there's at least one free space between a (cw front) and b (ccw front)."""
        if a is None or b is None: return False
        # going clockwise from a to b
        si = a
        for _ in range(n):
            if si == b: return True
            if si not in occupied: return True
            si = (si + 1) % n
        return False

    # Black: advance CW front
    if pc["black"] > 0:
        if not lv["closed_cw"]:
            ns = next_free_cw(lv["front_cw"])
            ccw_pos = next_free_ccw(lv["front_ccw"])
            if ns is not None and (ccw_pos is None or ns != ccw_pos or len(occupied) < n - 1):
                # Make sure we're not consuming the last space the ccw front needs
                if ns not in occupied:
                    moves.append(("black", "cw"))
        if not lv["closed_ccw"]:
            ns = next_free_ccw(lv["front_ccw"])
            cw_pos = next_free_cw(lv["front_cw"])
            if ns is not None and ns not in occupied:
