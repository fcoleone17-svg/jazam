...
def valid_moves(G):
    ...
    for lv, s, meta in slots:
        ...
        if pc["blue"] > 0 and not G["last_was_blue"] and not seed and meta != "forced":
...
def do_play(G, ptype, lv, space, meta, mode):
    ...
    if ptype == "blue" and not paying:
        ...
        if cel and lv < 5:
            ...
            G["last_was_blue"] = False   # la cadena la corta "la entrada no puede ser azul"
            G["forced"] = nxt
            if nxt is None: G["cp"] = next_cp(G)   # repite turno: subes tú
