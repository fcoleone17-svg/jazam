...
def do_play(G, ptype, lv, space, meta, mode):
    ...
    if ptype == "blue" and not paying:
        ...
        if cel and lv < 5:
            ...
            G["forced"] = nxt
            G["cp"] = next_cp(G)      # abre la puerta; el siguiente entra primero
...
with tab_rules:
    ...
    if ES:
        ...
        with c3: st.markdown('<div class="rule-box" style="text-align:center;"><b>🔵 Azul</b><br><small>Va en cualquier espacio · solo en celeste puntúa y abre el nivel siguiente.</small></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="rule-box">🔵 <b>La azul abre la puerta, pero el rival cruza primero.</b> Al colocarla en un celeste te llevas sus puntos y se abre el nivel siguiente — pero es <b>el jugador siguiente</b> quien coloca la primera pelota ahí, en el espacio de entrada. Ese es el precio de subir.</div>', unsafe_allow_html=True)
        st.divider()
        ...
        with c3: st.markdown('<div class="rule-box" style="text-align:center;"><b>🔵 Blue</b><br><small>Any space · only on a celestial does it score and open the next level.</small></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="rule-box">🔵 <b>Blue opens the door, but your opponent walks through first.</b> Playing it on a celestial scores its points and opens the next level — but the <b>next player</b> places the first bead there, at the entry space.</div>', unsafe_allow_html=True)
        st.divider()
        st.markdown("### Scoring")
