            ...
            # ── Turno normal: primero dirección, luego color ──
                ...
                if selected_dir is None:
                    if two_options:
                        ...
                        with c1:
                            st.markdown("""<style>
div[data-testid="stButton"] button.cw-btn {
    background:#22A022!important;border-color:#22A022!important;
    color:white!important;font-size:1.4rem!important;padding:12px!important;}
</style>""",unsafe_allow_html=True)
                            if st.button("↻",key="pick_cw",use_container_width=True,
                                        help=f"esp {t_cw2+1}",type="primary"):
                                st.session_state["selected_dir"]="cw"; st.rerun()
                        with c2:
                            if st.button("↺",key="pick_ccw",use_container_width=True,
                                        help=f"esp {t_ccw2+1}",type="secondary"):
                                st.session_state["selected_dir"]="ccw"; st.rerun()
                        # Color hint under buttons
                        st.markdown(
                            '<div style="display:flex;gap:8px;margin-top:4px;">'
                            '<div style="flex:5;text-align:center;font-size:11px;color:#22A022;">● verde</div>'
                            '<div style="flex:1;"></div>'
                            '<div style="flex:5;text-align:center;font-size:11px;color:#FF6B00;">● naranja</div>'
