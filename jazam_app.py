...
def valid_moves(G):
    ...
    cel = is_celestial(lv, si) and not G["blocking_space"]
    moves = []
    # Check both directions for the next space
    if pc["black"] > 0:
    ...
    if pc["blue"] > 0 and cel and lv < 5:
        moves.append(("blue", "cw"))
        moves.append(("blue", "ccw"))
    return moves
...
def render_board_svg(G):
    ...
    for li in range(6):
        ...
        for si in range(n):
            ...
            # Highlight next spaces to place
            is_next_cw = (li==nlv and si==nxt(nlv,nsi,"cw") and not G["over"])
            is_next_ccw = (li==nlv and si==nxt(nlv,nsi,"ccw") and not G["over"])
