            ...
            # ── Turno normal: primero dirección, luego color ──
                ...
                if selected_dir is None:
                    if two_options:
                        ...
                        # Capturar clic via query params
                        qp=st.query_params.get("dir","")
                        if qp in ("cw","ccw"):
                            st.session_state["selected_dir"]=qp
                            st.query_params.clear()
                            st.rerun()
                        components.html(f"""<!DOCTYPE html><html><body style="margin:0;padding:4px 0;">
<div style="display:flex;gap:16px;">
  <button onclick="go('cw')" style="flex:1;height:56px;font-size:1.8rem;background:#22A022;color:white;border:none;border-radius:10px;cursor:pointer;font-weight:bold;">↻</button>
  <button onclick="go('ccw')" style="flex:1;height:56px;font-size:1.8rem;background:#FF6B00;color:white;border:none;border-radius:10px;cursor:pointer;font-weight:bold;">↺</button>
</div>
<script>
function go(d){{
  const u=new URL(window.parent.location.href);
  u.searchParams.set('dir',d);
  window.parent.location.href=u.toString();
                ...
                # Paso 2: elegir color
                    ...
                    for i,t in enumerate(types_avail):
                        with cols[i]:
                            if st.button(labels[t],use_container_width=True,key=f"col_{t}",type="primary"):
                                do_play(G,t,d,mode)
