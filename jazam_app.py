...
def do_play(G, ptype, lv, space, meta, mode):
    ...
    if ptype == "blue" and not paying:
        ...
        if cel and lv < 5:
            ...
            add_log(G, who, f"{tn} {lvtag}{space+1} +{pts} → Nv{lv+2}", pts)
            G["last_was_blue"] = True
            G["forced"] = nxt
            if nxt is None: G["cp"] = (cp+1) % G["np"]
            check_end(G, mode); return                # repite turno
        add_log(G, who, f"{tn} {lvtag}{space+1}" + (f" +{pts}" if pts else ""), pts)
    ...
    add_log(G, who, f"{tn} {lvtag}{space+1}{note}", 0)
    ...
    if eff == "white":
        if G["variant"] == "classic":
            ...
            nb = empty_neighbor(G["board"], lv, space)
            if nb is not None: G["toll_at"] = (lv, nb)
...
def check_end(G, mode):
    ...
    if G["board"][5][0] is not None and not G["board"][5][0].get("scored"):
        ...
        G["winner"] = best_by_points(G)      # el centro da +12; gana quien más puntos tenga
        return
    cp = G["cp"]; pc = G["pieces"][cp]
...
def best_by_points(G):
    ...
    return pick_best(G, list(range(G["np"])))
...
def pieces_html(G, p):
    pc = G["pieces"][p]
    if G["np"] == 4: mx = {"black":10, "white":3, "blue":1}
    elif G["variant"] == "classic": mx = {"black":20, "white":6, "blue":2}
    else: mx = {"black":20, "white":6, "blue":2}
    rows = ""
...
with tab_rules:
    ...
    if ES:
        ...
        st.markdown('<div class="rule-box">Llegar al <b>centro</b> da +12 y <b>termina la partida</b> — pero no gana por sí solo: gana quien tenga <b>más puntos</b>.<br><br>Si un jugador <b>no tiene pelotas para colocar en su turno</b>, pierde. Entre el resto, gana quien tenga más puntos. En <b>parejas</b>, si un jugador se queda sin pelotas, pierde su equipo.</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="rule-box">Reaching the <b>center</b> scores +12 and <b>ends the game</b> — but doesn\'t win by itself: <b>most points wins</b>.<br><br>A player with <b>no beads to place on their turn</b> loses.</div>', unsafe_allow_html=True)
...
with tab_game:
    ...
    st.markdown(f'<div style="width:100%;max-width:440px;margin:6px auto;">{render_board_svg(G, slots, sel)}</div>',
                unsafe_allow_html=True)
    ...
    if not G["over"]:
        ...
        if human and moves:
            ...
            st.info("🤖 " + ("Pensando…" if ES else "Thinking…"), icon="⏳")
            time.sleep(0.5); ai_move(G, mode)
            st.session_state.pop("sel", None); st.rerun()
