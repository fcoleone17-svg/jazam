...
INIT_PIECES = {"black": 22, "white": 8, "blue": 2}
CELESTIALS = {
...
def render_clickable_board(G, clickable_spaces):
    """
    Renders SVG board inside an HTML component.
    clickable_spaces: list of (li, si) tuples that are clickable.
    Returns the clicked (li, si) or None via st.session_state['board_click'].
    """
    svg = render_board_svg(G)
    # Build JS click map: circle index -> (li, si)
    SIZE=440; cx=cy=SIZE//2
    radii=[int(cx*r) for r in [0.91,0.74,0.57,0.41,0.26,0.0]]

    click_areas = []
    for li, si in clickable_spaces:
        if li==5: x,y=cx,cy; r=22
        else:
            a=-math.pi/2+(si/LEVELS[li])*math.pi*2
            x=cx+radii[li]*math.cos(a); y=cy+radii[li]*math.sin(a); r=10
        click_areas.append({"li":li,"si":si,"x":round(x,1),"y":round(y,1),"r":r+6})

    import json
    areas_json=json.dumps(click_areas)

    html=f"""
<div style="position:relative;width:100%;max-width:440px;margin:0 auto;">
  {svg.replace('<svg ','<svg id="jazam-board" style="cursor:pointer;" ')}
  <canvas id="click-layer" style="position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:all;"></canvas>
</div>
<script>
const areas = {areas_json};
const canvas = document.getElementById('click-layer');
const svgEl = document.getElementById('jazam-board');
...
canvas.addEventListener('click', function(e) {{
  ...
  for (const a of areas) {{
    ...
    if (Math.sqrt(dx*dx + dy*dy) <= a.r) {{
      window.parent.postMessage({{type:'streamlit:setComponentValue', value: a.li + ',' + a.si}}, '*');
      break;
    }}
  }}
}});
...
"""
    return components.html(html, height=460, key="board_click_comp")
...
def pieces_html(pc):
    rows=""
    for t,mx in [("black",22),("white",8),("blue",2)]:
...
with tab_game:
    ...
    st.markdown(f'<div style="text-align:right;margin-top:-4px;"><span style="font-size:11px;background:#EAF3DE;color:#27500A;border:0.5px solid #97C459;border-radius:10px;padding:2px 9px;">6 {"niveles" if ES else "levels"} · 22⚫ · 8🟡 · 2🔵</span></div>',unsafe_allow_html=True)
    st.divider()
    ...
    if not G["over"]:
        ...
        if is_human and moves:
            ...
            selected=st.session_state.get("selected_color",None)
            ...
            # ── Turno normal: 2 pasos ──
                ...
                # Paso 1: elegir color
                if selected is None:
                    types_avail=list(dict.fromkeys(t for t,d in moves))
                    st.markdown(f"**{'Elige color:' if ES else 'Choose color:'}**")
                    cols=st.columns(len(types_avail))
                    labels={"black":f"⚫ ({pc['black']})","white":f"🟡 ({pc['white']})","blue":f"🔵 ({pc['blue']})"}
                    for i,t in enumerate(types_avail):
                        with cols[i]:
                            if st.button(labels[t],use_container_width=True,key=f"sel_{t}"):
                                if two_options:
                                    st.session_state["selected_color"]=t; st.rerun()
                                else:
                                    d="cw" if t_cw2 is not None else "ccw"
                                    do_play(G,t,d,mode); st.session_state.pop("selected_color",None); st.rerun()

                # Paso 2: clic en el tablero
                else:
                    t=selected
                    lbl_t={"black":"⚫","white":"🟡","blue":"🔵"}[t]
                    st.markdown(f"**{lbl_t} — {'toca el espacio en el tablero' if ES else 'tap the space on the board'}** &nbsp; "
                                f"<small><a href='#' onclick='window.dispatchEvent(new CustomEvent(\"cancel_sel\"))' style='color:#888;'>✕ {'cancelar' if ES else 'cancel'}</a></small>",
                                unsafe_allow_html=True)
                    if st.button("✕ Cancelar" if ES else "✕ Cancel", key="cancel_sel", use_container_width=False):
                        st.session_state.pop("selected_color",None); st.rerun()

                    # Build clickable spaces
                    clickable=[]
                    lv2=G["lv"]
                    if t_cw2 is not None and can(t,"cw"): clickable.append((lv2, t_cw2))
                    if t_ccw2 is not None and can(t,"ccw") and t_ccw2!=t_cw2: clickable.append((lv2, t_ccw2))

                    click_result=render_clickable_board(G, clickable)
                    if click_result:
                        try:
                            li_c,si_c=map(int,str(click_result).split(","))
                            if t_cw2 is not None and si_c==t_cw2 and li_c==lv2:
                                do_play(G,t,"cw",mode); st.session_state.pop("selected_color",None); st.rerun()
                            elif t_ccw2 is not None and si_c==t_ccw2 and li_c==lv2:
                                do_play(G,t,"ccw",mode); st.session_state.pop("selected_color",None); st.rerun()
                        except: pass
